...
export class LarkNotify extends NotifyBase implements INotifyBase {
   ...
   * @returns Promise
   */
  async send(message: LarkMessage): Promise<INotifyResult> {
    if (this.secret) {
   ...
   * @returns Promise
   */
  async sendText(text: string | LarkTextMessage): Promise<INotifyResult> {
    let msg: LarkMessage = {
      msg_type: 'text',
      content: { text: '' },
    }
   ...
   */
  async sendImage(
    image_key: string | LarkImageMessage
   ...
   * @returns Promise
   */
  async sendPost(post: LarkPostMessage): Promise<INotifyResult> {
    let msg: LarkMessage = {
      msg_type: 'post',
      content: post,
    }
   ...
   * @returns Promise
   */
  async sendCard(card: LarkCardMessage): Promise<INotifyResult> {
    let msg: LarkMessage = {
